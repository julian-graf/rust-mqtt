...
use rust_mqtt::{
    ...
    client::{
        Client,
        event::Event,
        options::{ConnectOptions, DisconnectOptions, PublicationOptions, TopicReference},
    },
    config::KeepAlive,
...
pub async fn mqtt(
    ...
    loop {
        ...
        let broker = IpEndpoint::new(IpAddress::v4(192, 168, 0, 107), port);
        ...
        let mut client: Client<'_, _, _, 1, 1, 0, 0, 0> = Client::new(&mut buffer);
        ...
        match client.connect(transport, &connect_options, None).await {
            ...
            Err(e) => {
                ...
                let mut n = client.abort().await.unwrap();

                match n.close().await {
                    Ok(_) => defmt::info!("[TCP] connection closed"),
                    Err(e) => {
                        defmt::error!("[TCP] error occured closing connection: {:?}", e)
                    }
                }
        ...
        match client.disconnect(&DisconnectOptions::new()).await {
            ...
            Err(e) => {
                defmt::info!("[MQTT] failed to disconnect: {:?}", e);
                let mut n = client.abort().await.unwrap();
