...
impl<'b> PacketReceiver<'b> {
    ...
    pub async fn poll<'r, R: Read>(
        &'r mut self,
        read: &mut R,
    ) -> Result<PacketDecoder<'r>, ReaderError> {
        ...
        self.initialized = 0;
        let buf = &self.buf[..remaining_len];

        Ok(PacketDecoder::new(header, buf))
    }
...
impl<'b> PacketDecoder<'b> {
    pub fn new(header: FixedHeader, buf: &'b [u8]) -> Self {
        Self {
            header,
            buf,
            pos: 0,
    ...
    pub fn take_bytes(&mut self, len: usize) -> Result<&'b [u8], InsufficientLen> {
        let end = self.pos + len;
