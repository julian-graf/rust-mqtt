...
use crate::{
    ...
    fmt::{debug, debug_assert, error, panic, warn},
    header::FixedHeader,
...
impl<'b, N: Transport, B: BufferProvider<'b>> Raw<'b, N, B> {
    ...
    ///
    /// This expects the network to be in neither `Ok()` nor `Terminated` state
    pub async fn abort(&mut self) -> Result<N, AbortError> {
        debug_assert!(
            !self.n.is_terminated(),
            "network must be in Faulted(n, reason_code) or Completed(n) state to disconnect due to an error."
        );
        ...
        match n {
            NetState::Ok(n) => {
                ...
                if cfg!(debug_assertions) {
                    panic!(
                        "network must be in DueDisconnect(n, reason_code) or Inactive(n) state to abort due to an error."
                    );
                }

                // let packet = DisconnectPacket::<0>::new(
                ...
                // debug!(
                //     "sending DISCONNECT packet with reason code: {:?}",
                //     ReasonCode::UnspecifiedError
                // );
